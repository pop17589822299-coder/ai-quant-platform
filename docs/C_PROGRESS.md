...
**C 收尾已确认：main `5fa91ea524a403b31cc5980480aeb7c25898479b` 已包含预热修复，C在实际main上三个入口场景全部通过。PR #10/#12/#14均已合并；C当前无算法/接口阻塞。原量化/固定输入树未变，原九组与MySQL证据保持原范围；本轮新增入口证据为SQLite/合成行情及日历。PR #16仅新增接线测试，C独立3项通过，由D决定是否作为常驻守卫合入。**

```mermaid
flowchart LR
    D["main 5fa91ea<br/>核心/固定包等同 bcbd559"] --> C["原557 tests · 固定九组通过"]
    C --> R["SQLite/MySQL 各12场景通过<br/>原两项 C 阻塞关闭"]
    R --> W["预热修复已进入main<br/>C在main复验3场景通过"]
    W --> A["D已记录用户验收通过<br/>A未独立验候选关Mock页面"]
    A --> U["C收尾关闭<br/>PR16常驻测试由D决定"]
```
...
| B 预热脚本 | ✅ 已进入main 5fa91ea；C独立执行实际入口三个场景通过，旧脚本负向对照仍复现 |
| 页面/用户验收 | D 在87f1ed5记录用户验收通过并代理关闭A门禁；A未独立验候选关Mock页面，非C实测 |
| 整体 V2 | D已宣布收尾，PR #10/#12/#14均合并；C当前无阻塞，PR #13属后续A范围 |
| PR #16 | 仅新增测试；C在19c9276运行3 passed，建议保留行为守卫，合并由D决定 |
...
### 2026-09-22：main 5fa91ea 实际入口复验通过，C 收尾关闭

已核对D 5770768828与B更正5770848782/5770848898；PR #14确已合并，旧“待合/取代/关闭”建议作废。main中脚本与a89c2ea逐字节相同；C在实际main重新执行入口三个场景通过，并独立执行PR #16新增测试3项通过。原核心/固定包树未变，不重跑未受影响套件。详见[最终确认报告](C_V2_MAIN_CLOSURE_20260922.md)与[原始结果](evidence/c-warm-entry-20260922/main-5fa91ea-result.json)。本轮证据在C分支；PR #12已关闭，不能宣称后续C提交自动进入main。以下均为历史时点状态。

### 2026-09-22：87f1ed5 收尾记录与审阅请求确认

**末次复查更正**：PR #10于03:07:46Z合并为`8dbe2e47b890e2992b2feb8cdb4046c64ed34827`；PR #12于03:08:32Z以C头`5aa24aa`合并为`5a1247e61e6d3197c427f0cdcfa88d0a24d8c437`。前次API快照的Draft/未合并状态已过时。C后续确认记录`3f1e13e`及本次文档更新不在已关闭的PR #12中，不能宣称随原PR进入main；D可在最终文档收尾时同步。PR #14仍未合并，main脚本blob核对仍旧4397bbd。下文保留本轮先前快照作为时点记录。
